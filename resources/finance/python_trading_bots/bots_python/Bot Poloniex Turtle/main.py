...
polo=poloniex.Poloniex('1M2S9KMQ-C00P2P89-HAQUF7AF-1C73GC08','6818b13dd8047b108d85c8acb236d2462c3927ba9f856e812c90a2f5c58ae6d04a02aec07476dece8ccc81abb023d4ba279701a8c21aacbccf7da282adfcfe81')
...
def broker_update():
    ...
    try:
        bars1=polo.directpublic('https://poloniex.com/public?command=returnChartData&currencyPair=BTC_ETH&start=1405699200&end=1469404800&period=900')
        bars2=polo.directpublic('https://poloniex.com/public?command=returnChartData&currencyPair=BTC_ETC&start=1469404801&end=9999999999&period=900')
        bars=bars1+bars2
    ...
    if shared.CheckCode==False:
        print("   ")
        print("********************************************************************************************************************")
        print(" T I C K = %s  [%s H] [Model:%s] [POLONIEX ETCBTC 15m]" % (shared.TICK, format(shared.TICK/4,'.2f'), shared.ALGO))
        print("********************************************************************************************************************")
...
def main_algo():
    ...
    if shared.ALGO=="ZF4":
        ...
        if shared.vZF4[-1]<shared.Th2:
            shared.sig=-1                
        print("--------------------------------------------------------------------------------------------------------------------")
        print("[ Bot Poloniex ZF4 ETCBTC 15m - %s ] : LAST SIGNAL = %s [Sell %s < ZF4 = %s  > Buy %s ]" %(time.strftime("%Y-%m-%d %H:%M:%S"), shared.sig,shared.Th2 , format(shared.vZF4[-1],'.4f') , shared.Th1))
        print("--------------------------------------------------------------------------------------------------------------------")
