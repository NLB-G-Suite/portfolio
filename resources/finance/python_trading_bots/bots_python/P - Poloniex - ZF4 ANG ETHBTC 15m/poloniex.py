...
class Poloniex:
  ...
  def api(self, command, args={}):
    ...
    if command in PRIVATE_COMMANDS:
      ...
      url, args['nonce'] = ['https://poloniex.com/tradingApi', int(time.time()*2000000)] #42
      ...
      try:
        js=json.loads(ret.read().decode(encoding='UTF-8'))
      except:
        print("WARNING: There was a problem in the call to the API (api)")
      return js
  ...
  def directpublic(self,url):
    ret = urlopen(Request(url))
    js=0
    try:
      js=json.loads(ret.read().decode(encoding='UTF-8'))
    except:
      print("WARNING: There was a problem in the call to the API (directpublic)")
    return js
    
  #def directprivate(self,url):
  #  ret=urlopen(Request(url))
  #  return json.loads(ret.read().decode(enconding='UTF-8'))