    ...
    while buying:
        ...
        if shared.SPLIT>1 and float(shared.curr/shared.price) > max(1,float(orderSize*1.2)) and shared.curr > shared.MinOrderCurr:
            ...
            if shared.curr/shared.price > 0.01:
                print("Iceberg last order. Trying to spend the last %s %s buying %s at price %s" % (shared.curr, shared.PairCurr, shared.curr*0.95, shared.price*(1+20*shared.PriceOverlap)))
                print(okcoinSpot.trade('btc_cny', 'buy' , shared.price*(1+20*shared.PriceOverlap) , shared.curr*0.99 ))
            else:
...
def main_algo():
    ...
    if shared.ALGO=="ZF4":
        ...
        if shared.vZF4[-1]<shared.Th2:
            shared.sig=-1                
        print("--------------------------------------------------------------------------------------------------------------------")
        print("[ Bot Okcoin ZF4 BTCCNY 15m - %s ] : LAST SIGNAL = %s [Sell %s < ZF4 = %s  > Buy %s ]" %(time.strftime("%Y-%m-%d %H:%M:%S"), shared.sig,shared.Th2 , format(shared.vZF4[-1],'.4f') , shared.Th1))
        print("--------------------------------------------------------------------------------------------------------------------")
