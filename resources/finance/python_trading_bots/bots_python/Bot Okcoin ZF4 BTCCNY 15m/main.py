    ...
    while buying:
        ...
        if shared.SPLIT>1 and float(shared.curr/shared.price) > max(1,float(orderSize*1.2)) and shared.curr > shared.MinOrderCurr:
            ...
            if shared.curr/shared.price > 0.01:
                print("Iceberg last order. Trying to spend the last %s %s buying %s at price %s" % (shared.curr, shared.PairCurr, shared.curr/shared.price*0.95, shared.price*(1+20*shared.PriceOverlap)))
                print(okcoinSpot.trade('btc_cny', 'buy' , shared.price*(1+20*shared.PriceOverlap) , shared.curr/shared.price*0.95 ))
            else:
    ...
    while selling:
        ...
        if shared.SPLIT>1 and shared.assets > max(1,orderSize*1.2):
            ...
            if shared.assets>0.01:
                print("Iceberg last order. Trying to sell the last %s %s for %s at price %s" % (shared.assets, shared.PairAsset,shared.price*(1-20*shared.PriceOverlap),shared.assets*0.99))
                print(okcoinSpot.trade('btc_cny', 'sell' , shared.price*(1-20*shared.PriceOverlap)  , shared.assets*0.99 ))
            else:
                print("The exchange does not accept orders smaller than 0.01")
            selling=False
...
def broker_update():
    ...
    if shared.CheckCode==False:
        print("   ")
        print("********************************************************************************************************************")
        print(" T I C K = %s  [%s H] [BOT: %s]" % (shared.TICK, format(shared.TICK/4,'.2f'), shared.BOT_NAME))
        print("********************************************************************************************************************")
...
def main_algo():
    ...
    if shared.ALGO=="ZF4":
        ...
        if shared.vZF4[-1]<shared.Th2:
            shared.sig=-1                
        print("--------------------------------------------------------------------------------------------------------------------")
        print("[ %s - %s ] : LAST SIGNAL = %s [Sell %s < ZF4 = %s  > Buy %s ]" %(shared.BOT_NAME,time.strftime("%Y-%m-%d %H:%M:%S"), shared.sig,shared.Th2 , format(shared.vZF4[-1],'.4f') , shared.Th1))
        print("--------------------------------------------------------------------------------------------------------------------")
