...
def broker_update():
    ...
    try:
        if os.name=='nt':
            shared.bars=genfromtxt('c:/teleferi/CSV_Tickers/OK-BTCCNY-15m.csv',delimiter=';',dtype=[('d','S16'),('o','<f8'),('c','<f8'),('h','<f8'),('l','<f8'),('v','<f8')],usemask=False)        
        else:
    ...
    try:
        if shared.CheckCode==False:
            if os.name=='nt':
                f=csv.writer(open("c:/teleferi/CSV_Tickers/stats_CR2LinRegSTD_Okcoin_15m.csv","a+"),delimiter=';')
